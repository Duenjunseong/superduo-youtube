    ...
    try:
        ...
        if not job.title:
            try:
                ...
                job.progress = 5 # 영상 정보 가져오기 완료 후 5%
                job.save()
                logger.info(f"영상 제목 가져옴: {job.title}, progress=5%")
        ...
        extractor_args = 'youtube:player_client=ios' # 기본값
        ...
        cmd = [
            'yt-dlp', '-f', format_selector, '--no-playlist', '--no-warnings', 
            '--no-check-certificate', 
            '--user-agent', 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15',
            '--embed-chapters', '--write-info-json', '-o', output_template, url,
            '--extractor-args', extractor_args
        ]
        
        job.progress = 10 # 다운로드 명령 실행 직전 10%
        job.save()
        logger.info(f"다운로드 명령 실행 준비: progress=10%, Command: {' '.join(cmd)}")
        
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            logger.info(f"yt-dlp 실행 성공 - stdout: {result.stdout}")
            if result.stderr: logger.warning(f"yt-dlp stderr: {result.stderr}")
            
            job.progress = 70 # yt-dlp 실행 성공 (파일 다운로드 완료) 후 70%
            job.save()
            logger.info(f"파일 다운로드 완료 (yt-dlp): progress=70%")

        except subprocess.CalledProcessError as e:
            logger.error(f"yt-dlp 실행 실패: {e}, stderr: {e.stderr}, stdout: {e.stdout}")
             # 1차 실패 시 fallback 포맷으로 재시도
            logger.info("더 간단한 포맷으로 재시도합니다...")
            fallback_cmd = [
                'yt-dlp', '-f', '270+251/270+140/232+251/232+140/18',
                '--no-playlist', '--no-warnings', '--no-check-certificate',
                '--user-agent', 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15',
                '-o', output_template, url, '--extractor-args', 'youtube:player_client=ios'
            ]
            ...
            try:
                ...
                job.progress = 70 # Fallback 성공 시 70%
                job.save()
                logger.info(f"파일 다운로드 완료 (Fallback iOS): progress=70%")
            except subprocess.CalledProcessError as fallback_e:
                logger.error(f"iOS Fallback 다운로드도 실패, Android로 재시도: {fallback_e}")
                android_cmd = [
                    'yt-dlp', '-f', '18', '--no-playlist', '--no-warnings',
                    '--no-check-certificate',
                    '--user-agent', 'com.google.android.youtube/17.36.37 (Linux; U; Android 11)',
                    '-o', output_template, url, '--extractor-args', 'youtube:player_client=android'
                ]
                try:
                    ...
                    job.progress = 70 # Android Fallback 성공 시 70%
                    job.save()
                    logger.info(f"파일 다운로드 완료 (Fallback Android): progress=70%")
        ...
        logger.info(f"임시 파일 정보: {temp_file_path}, 크기: {file_size} bytes")

        job.progress = 80 # 파일 처리 준비 단계 80%
        job.save()
        logger.info(f"파일 처리 준비: progress=80%")
        ...
        if file_ext in ['.mp4', '.mkv', '.webm', '.avi']: file_type = f'video/{file_ext[1:]}'
        elif file_ext in ['.mp3', '.m4a', '.wav', '.ogg']: file_type = f'audio/{file_ext[1:]}'
        else: file_type = 'application/octet-stream'
        
        relative_file_path = os.path.relpath(permanent_file_path, media_root)
        
        job.progress = 95 # DB 저장 직전 95%
        job.save()
        logger.info(f"DB 저장 준비: progress=95%")

        file_obj = File.objects.create(
            job=job, filename=permanent_filename, file_path=relative_file_path,
            file_size=file_size, file_type=file_type
        )
        logger.info(f"파일 정보 저장: file_id={file_obj.id}, relative_path={relative_file_path}")
        ...
        return {
            'status': 'success', 'job_id': job.id, 'file_id': file_obj.id,
            'file_path': file_obj.file_path, 'filename': file_obj.filename, 'file_size': file_size
        }
        ...
        if job:
            ...
            job.progress = 0 # 실패 시 진행률 초기화 또는 특정 값 설정
            job.save()
